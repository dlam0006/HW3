...
function selectBusesByDriver($did) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT s.shift_id, b.bus_id, s.route_id, bus_name, capacity, shift_days, start_time, end_time FROM `Bus` b join Shift s on b.bus_id = s.bus_id where s.driver_id=?");
